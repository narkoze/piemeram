    ...
    WithMapping
{
    ...
    public static function beforeExport(BeforeExport $event)
    {
        ...
        $writer->setCompany(config('app.name'));
        ...
        foreach (self::filters() as $i => $rules) {
            ...
            switch (key($rules)) {
                ...
                case 'betweenDates':
                    ...
                    $column->createRule()
                        ->setRule(Rule::AUTOFILTER_COLUMN_RULE_LESSTHANOREQUAL, Date::dateTimeToExcel(Carbon::parse($rules['betweenDates'][0])))
                        ->setRuleType(Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);

                    $column->setJoin(Column::AUTOFILTER_COLUMN_JOIN_AND);

                    $column->createRule()
                        ->setRule(Rule::AUTOFILTER_COLUMN_RULE_GREATERTHANOREQUAL, Date::dateTimeToExcel(Carbon::parse($rules['betweenDates'][1])))
                        ->setRuleType(Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);
                    break;
            }
        }
